        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    <x-nav-link href="{{ route('dashboard.restaurateur') }}" :active="request()->routeIs('dashboard.restaurateur')">
                        {{ __('Mon Espace Pro') }}
                    </x-nav-link>
                    @endif
                    @endauth
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                ...
                <div class="ms-3 relative">
                    <x-dropdown align="right" width="48">
                        ...
                        <x-slot name="content">
                            <div class="block px-4 py-2 text-xs text-gray-400">{{ __('Gérer le compte') }}</div>
                            <x-dropdown-link href="{{ route('profile.show') }}">{{ __('Profil') }}</x-dropdown-link>

                            @if(auth()->user()->role === 'restaurateur')
                            <x-dropdown-link href="{{ route('dashboard.restaurateur') }}">{{ __('Dashboard Pro') }}</x-dropdown-link>
                            @endif
                            ...
                            <form method="POST" action="{{ route('logout') }}" x-data>
                                @csrf
                                <x-dropdown-link href="{{ route('logout') }}" @click.prevent="$root.submit();">{{ __('Déconnexion') }}</x-dropdown-link>
                            </form>
                        </x-slot>
                    </x-dropdown>
                </div>
                ...
                <div class="flex items-center gap-4">
                    ...
                    <a href="{{ route('register') }}" class="px-4 py-2 text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-500 rounded-lg shadow transition-all">Inscription</a>
                </div>
                @endauth
            </div>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        ...
        <div class="pt-4 pb-1 border-t border-gray-200">
            ...
            <div class="mt-3 space-y-1">
                ...
                @if(auth()->user()->role === 'restaurateur')
                <x-responsive-nav-link href="{{ route('dashboard.restaurateur') }}">
                    {{ __('Dashboard Pro') }}
                </x-responsive-nav-link>
                @endif
